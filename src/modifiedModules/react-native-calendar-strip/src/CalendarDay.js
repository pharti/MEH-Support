...
class CalendarDay extends Component {
  ...
  render() {
    ...
    let day;
    const gradientColor = selected ? ['#639590', '#9BD5D0'] : ['#fff', '#fff'];
    const _dayColor = selected ? '#639590' : '##9F9D9D';
    const getColorValue = () => {
      if (date.date() == this.state.today && selected) {
        return '#fff';
      } else if (date.date() == this.state.today) {
        return '#308D85';
      } else if (selected) {
        return '#fff';
      } else {
    ...
    if (DayComponent) {
      ...
      let _borderStyle =
        date.date() == this.state.today
          ? {borderWidth: 3, borderColor: '#BBD6D4'}
          : {};
      day = (
        <TouchableOpacity
          style={{
